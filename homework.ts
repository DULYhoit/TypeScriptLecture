...
function testge<Mytype extends string | string[]>(x:Mytype) {
  console.log(x.length);
}
testge<string>('hello');
testge<string[]>(['hello','hi']);
...
function changeobj<Mytype extends string>(x:Mytype) {
 return JSON.parse(x);
}
...
//3.class 를 수정해봅시다.
class Person<Mytype extends number |string| string[]> {
  name : Mytype;
